...
export default function Header() {
  ...
  const handleMapAlert = () => {
    alert("KAKAO_MAP_SHARE_LINK을 설정하세요.");
  };
    ...
    <header className="header" data-reveal="down">
      <div className="brand">
        <img
          ...
          height="44"
        />
        <div className="title">대한예수교침례회 인천교회 농인선교부</div>
      </div>
      <div className="btns">
  {KAKAO_MAP_SHARE_LINK ? (
    <a
      ...
      title="카카오맵으로 길찾기"
    >
      {/* [수정] <span className="badge-kakao">...</span> 를 아래 svg로 교체 */}
      {/* [수정] <span className="pin">...</span> 은 삭제 */}
      <svg
      ...
      카카오맵 열기
          ...
          <button className="btn kmap" onClick={handleMapAlert}>
            <span className="badge-kakao" aria-hidden>
              K
            </span>{" "}
            카카오맵 열기 <span className="pin" aria-hidden />
          </button>
        )}
